...
/**
 * Created by Rene Argento on 21/12/18.
 */
public class Day21 {
    ...
    public static void main(String[] args) {
        ...
        executeInstructions(instructions, instructionPointer, 0);
    }

    private static void executeInstructions(List<String> instructions, int instructionPointer, int initialRegister0Value) {
        int[] registers = new int[6];
        registers[0] = initialRegister0Value;
        int criticalInstruction = 28;

        int nextInstructionId = 0;
        long valueThatCanHaltWithMostInstructions = 0;
        Set<Integer> valuesThatCanHaltTheProgram = new HashSet<>();
        ...
        while (true) {
            ...
            if (registers[instructionPointer] == criticalInstruction) {
                int valueThatCanHalt = registers[4];

                if (valuesThatCanHaltTheProgram.isEmpty()) {
                    System.out.println("Value that causes program to halt with fewest instructions: " + valueThatCanHalt);
                }

                if (valuesThatCanHaltTheProgram.contains(valueThatCanHalt)) {
                    System.out.println("Value that causes program to halt with most instructions: " + valueThatCanHaltWithMostInstructions);
                    return;
                }
                valuesThatCanHaltTheProgram.add(valueThatCanHalt);
                valueThatCanHaltWithMostInstructions = valueThatCanHalt;
            }
